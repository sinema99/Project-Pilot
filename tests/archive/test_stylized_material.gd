...
const STYLIZED := "res://shaders/archive/stylized.gdshader"
...
func test_hub_material_lifts_its_shadow_band() -> void:
	var mat: ShaderMaterial = load("res://resources/archive/stylized_material.tres")
	check(mat != null, "res://resources/archive/stylized_material.tres failed to load")
	if mat == null:
		return
...
# Spec (night-pass.md): the rim light the art-direction spec deferred. It exists so
# a dark character does not dissolve into a dark wall, and it ships OFF -- the day
# scene never asked for haloed edges, and a uniform that defaults on would re-tune
# every material that already exists.
func test_stylized_exposes_a_rim_light_shipped_off() -> void:
	var names := shader_uniform_names(STYLIZED)
	for required in ["rim_enabled", "rim_color", "rim_threshold", "rim_strength"]:
		check(names.has(required), "missing rim uniform '%s' (have: %s)" % [required, ", ".join(names)])
	eq(declared_default(STYLIZED, "rim_enabled"), "false", "the rim must ship off")
