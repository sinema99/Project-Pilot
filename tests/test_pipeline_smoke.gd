...
const GDSHADERS := [
	"res://shaders/stylized.gdshader",
	"res://shaders/gradient_sky.gdshader",
]
...
func test_outline_compute_shader_compiles_to_spirv() -> void:
	var shader_file: RDShaderFile = load("res://shaders/moebius_outline.glsl")
	check(shader_file != null, "res://shaders/moebius_outline.glsl failed to load")
	if shader_file == null:
		return
