...
## Crease lines (the normal-driven ones) are drawn only where two faces meet at
## more than this angle. A true dihedral angle in degrees, not a gradient
## magnitude: 30 inks hard corners while leaving shallow bevels and gentle
## surface curvature clean; lower it to pick up softer folds.
@export var normal_edge_angle_deg: float = 30.0
## Ink the silhouette where geometry meets the sky. Off by default -- the base
## look keeps the horizon clean -- but scenes where the terrain itself is the
## skyline (open desert, mountains) want that ridge outlined. Fades with
## distance like the other lines.
@export var ink_sky_silhouette: bool = false
...
## Luminance below which the grain ramps out to nothing, so pure black stays
## pure black instead of speckling -- additive grain has no negative half to
## spend down there, so it only ever lifts black off zero. 0.0 turns the ramp
## off and grains the whole frame evenly, which is the original behaviour.
@export var grain_luma_falloff: float = 0.0
...
func build_push_constant(size: Vector2i, grain_seed: float, time: float,
	...
	var values := PackedFloat32Array([
		...
		z_far,
		grain_luma_falloff,
		1.0 if ink_sky_silhouette else 0.0,
	])
